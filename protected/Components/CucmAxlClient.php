    ...
    public function client(): \SoapClient
    {
        if (is_null($this->client)) {
            try {
                ...
                $this->client =  new \SoapClient(realpath(ROOT_PATH . '/AXLscheme/' . $this->schema() . '/AXLAPI.wsdl'), [
                    'trace' => true,
                    'exception' => true,
                    'location' => 'https://' . $this->ip . ':8443/axl',
                    'login' => $this->login,
                    'password' => $this->password,
                    'stream_context' => $this->context(),
                ]);
            } catch (\Throwable $e) {
                throw new \Exception('AXL connection error');
            }
        }
        return $this->client;
    }
