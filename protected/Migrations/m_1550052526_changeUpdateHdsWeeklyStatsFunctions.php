...
class m_1550052526_changeUpdateHdsWeeklyStatsFunctions
    extends Migration
{
              ...
              EXECUTE \'
                CREATE VIEW \' || view_name || \' AS
                  SELECT * FROM crosstab(\'\'
                    SELECT
                      (SELECT cast(to_char(min(t2.date), \'\'\'\'YYYY-MM-DD\'\'\'\') AS citext) AS date
                        FROM hds.hds_weekly_agents_dn_statistics AS t2
                        WHERE t2.year=t1.year AND t2.week=t1.week
                        GROUP BY t2.year, t2.week) AS date,
                      t1.prefix,
                      count(t1.dn) AS amount_dn
                    FROM hds.hds_weekly_agents_dn_statistics AS t1
                    GROUP BY t1.year, t1.week, t1.prefix
                    ORDER BY t1.year, t1.week, t1.prefix
                \'\', \'\'
                ...
                CREATE VIEW \' || view_name || \' AS
                  SELECT * FROM crosstab(\'\'
                    SELECT
                      ...
                      count(t1.dn) AS amount_dn
                    FROM hds.hds_weekly_agents_dn_statistics AS t1
                    WHERE cast(t1.prefix AS INTEGER) = \' || prefix_val || \'
                    GROUP BY t1.year, t1.week, t1."lotusId"
                    ORDER BY t1.week, t1."lotusId"
                \'\', \'\'
    ...
    {
        $sql['repair_function__hds.create_hds_weekly_stats_by_prefixes'] = '
            CREATE OR REPLACE FUNCTION hds.create_hds_weekly_stats_by_prefixes() RETURNS VOID AS $$
            DECLARE
              view_name citext := \'view.hds_weekly_stats_by_prefixes\';
              prefix citext;
        ...
        $sql['repair_function__hds.create_hds_weekly_stats_by_offices_on_prefix'] = '
            CREATE OR REPLACE FUNCTION hds.create_hds_weekly_stats_by_offices_on_prefix(prefix_val citext) RETURNS VOID AS $$
            DECLARE
              view_name citext := \'view.hds_weekly_stats_by_offices_on_prefix_\' || prefix_val;
              prefix_columns citext := \'\';
