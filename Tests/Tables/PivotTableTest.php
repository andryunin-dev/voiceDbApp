...
TAG;
    ...
    public function providerSelectStatement()
    {
        return [
            'withoutPreFilter' => [
                [],
                'SELECT "columnOne", (SELECT jsonb_object_agg(t2."columnTwo", t2.numbers) FROM ( SELECT "columnTwo", count("columnTwo") AS numbers FROM "ModelClass_1"  AS t3 WHERE "columnOne" = :columnOne_eq_0 GROUP BY "columnTwo" ORDER BY "columnTwo", "columnThree" ) AS t2 ) AS "columnTwoPivot", "columnThree" FROM "ModelClass_1" AS t1 GROUP BY "columnOne", "columnThree" ORDER BY "columnOne" ASC, "columnThree" ASC'
            ],
            'columnOne 1 val' => [
                ['columnOne' => ['eq' => ['c-1-v-0', 'c-1-v-1']]],
                'SELECT "columnOne", (SELECT jsonb_object_agg(t2."columnTwo", t2.numbers) FROM ( SELECT "columnTwo", count("columnTwo") AS numbers FROM "ModelClass_1"  AS t3 WHERE ("columnOne" = :columnOne_eq_0 OR "columnOne" = :columnOne_eq_1) GROUP BY "columnTwo" ORDER BY "columnTwo", "columnThree" ) AS t2 ) AS "columnTwoPivot", "columnThree" FROM "ModelClass_1" AS t1 WHERE ("columnOne" = :columnOne_eq_2 OR "columnOne" = :columnOne_eq_3) GROUP BY "columnOne", "columnThree" ORDER BY "columnOne" ASC, "columnThree" ASC'
            ],
            'columnOne 2 val' => [
                ['columnOne' => ['eq' => ['c-1-v-0']]],
                'SELECT "columnOne", (SELECT jsonb_object_agg(t2."columnTwo", t2.numbers) FROM ( SELECT "columnTwo", count("columnTwo") AS numbers FROM "ModelClass_1"  AS t3 WHERE "columnOne" = :columnOne_eq_0 GROUP BY "columnTwo" ORDER BY "columnTwo", "columnThree" ) AS t2 ) AS "columnTwoPivot", "columnThree" FROM "ModelClass_1" AS t1 WHERE "columnOne" = :columnOne_eq_1 GROUP BY "columnOne", "columnThree" ORDER BY "columnOne" ASC, "columnThree" ASC'
            ],
        ];
    }
